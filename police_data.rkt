...
(provide load-data
         parameters-and-police)

(define (load-data #:skip-rows [to-skip 0])
  (with-input-from-file "police.txt"
    (lambda ()
      ; Discard to-skip
...
(define (parameters-and-police df)
  (define n-cols (ncols df))
  (values (sub df 0 0 (nrows df) (sub1 n-cols)) (col df (sub1 n-cols))))

(module+ main
  (define-values (params pizzas) (parameters-and-police (load-data #:skip-rows 1)))
